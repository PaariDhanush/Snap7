...
1)Install Python 3.11.10 for Windows

2)install the libraries (In Windwos CMD)
  py -m pip install python-snap7
  py -m pip install Pillow

3) open Python IDLE Shell and create new file and copy the code,then save the file

4)just your values (PLC IP Adress, Rack number, Slot number, DB Area, DB Number)
---------------------------------------------------------------------------------------------
...
def read_plc_data():
    ...
    while True:
        try:
            ...
            # Read (for example, DB area, DB number, byte offset)
